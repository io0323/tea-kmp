...
@Composable
internal fun WalkThroughCoachMarkScreen(
    currentPage: Int,
    modelList: List<WalkThroughCoachMarkModel>,
    onPageChanged: (Int) -> Unit,
    onClickSkip: () -> Unit,
    onClickStart: () -> Unit,
) {
    val pagerState = rememberPagerState(pageCount = { modelList.size })
    val coroutineScope = rememberCoroutineScope()
    val model = modelList[currentPage]

    Scaffold(
        topBar = {
            if (currentPage == modelList.size) {
...
@Composable
fun CoachMark(modifier: Modifier = Modifier, targetPosition: Offset) {
    val circleSize: Dp by animateDpAsState(targetValue = 120.dp)
...
//                .padding(targetPosition)
        ) {
            Text("This is a CoachMark", modifier = Modifier.align(Alignment.Center))
        }
    }
}